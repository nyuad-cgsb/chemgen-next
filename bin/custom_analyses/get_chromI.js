    ...
    data.expDesigns.map(function (expDesignSet) {
        if (!lodash_1.isNull(expDesignSet) && !lodash_1.isEmpty(expDesignSet)) {
            expDesignSet.map(function (expDesignRow) {
                var obj = { assayExpGroup: expDesignRow.treatmentGroupId };
                or.push(obj);
                obj = { assayExpGroup: expDesignRow.controlGroupId };
                or.push(obj);
            });
        }
    });
    return new Promise(function (resolve, reject) {
        //TODO Also get counts!
        //TODO Paginate
        ...
        if (!lodash_1.isNull(expDesigns) && !lodash_1.isEmpty(expDesigns)) {
            var expAssays = lodash_1.filter(data.expAssays, function (expAssay) {
                return lodash_1.find(expDesigns, function (expDesign) {
                    return lodash_1.isEqual(Number(expAssay.assayExpGroup), Number(expDesign.controlGroupId)) ||
                        lodash_1.isEqual(Number(expAssay.assayExpGroup), Number(expDesign.treatmentGroupId));
                });
            });
            //Get the gene
...
function getExpDesign(genesList) {
    ...
    return new Promise(function (resolve, reject) {
        ...
        app.models.ExpAssay2reagent
            .find({ where: { or: or }, fields: { assayExpGroup: true, assayId: true } })
            .then(function (results) {
            ...
            //   return {expGroupId: expAssay2reagent.assayExpGroup};
            // });
            var expAssayIds = results.map(function (expAssay2reagent) {
                return { assayId: expAssay2reagent.assayId };
            });
            return app.models.ExpAssay.find({ where: { or: expAssayIds }, fields: { assayExpGroup: true } });
        })
            .then(function (results) {
            var expGroupIds = results.map(function (expAssay) {
                return { expGroupId: expAssay.assayExpGroup };
            });
